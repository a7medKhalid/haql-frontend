...
const ResponsiveNavLink = ({ active = false, children, ...props }) => (
    <Link {...props} href={props.href}>
        <a
            className={`block pl-3 pr-4 py-2 border-r-4 text-base font-medium leading-5 focus:outline-none transition duration-150 ease-in-out ${
                active
                    ? 'border-accent text-primary-text bg-accent/10'
